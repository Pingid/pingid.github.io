...
export default (posts) => {
  ...
  const rest = !top ? posts : posts.filter(x => x.title !== top.title).reduce((a, b) => {
    ...
    if (R.prop('coverImage', b) && R.length(R.prop('coverImage', b)) > 0) {
      ...
      if ( R.length(last) === 0 || (R.length(last) === 1 && !R.prop('coverImage', R.head(last))) ) return addToLast;
      return addNewLast;
    }
    // Does the last array have less than 3 items
    if ( R.length(last) < 3 ) {
      // does the array have two none image items or one or none items
      if (R.length(last) < 2 || (R.length(last) === 2 && !R.find(R.prop('coverImage'), last)) ) return addToLast;
      return addNewLast;
    }
    return addNewLast
  }, [[]])
